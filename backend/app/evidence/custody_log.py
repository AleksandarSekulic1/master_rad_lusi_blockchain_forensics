...
def custody_chain_for_transaction(case_id: str, tx_id: str) -> dict[str, Any] | None:
    ...
    # A TOKEN_APPROVAL/SYBIL_CLUSTER finding (see
    # app.features.case_token_approval_analysis.service.token_approval_custody_enrichment /
    # app.features.case_sybil_analysis.service.sybil_custody_enrichment) is the same,
    # deterministic fact every time this transaction is re-analysed - taken from whichever
    # access actually carried it (usually all of them, if any do), rather than assumed to
    # be on `last` specifically, since an unrelated access to the SAME transaction (e.g.
    # from a different analysis) never attaches one at all.
    ...
    sybil_evidence = next((entry.get('sybil_evidence') for entry in reversed(entries) if entry.get('sybil_evidence')), None)
...
def list_case_transactions(case_id: str) -> list[dict[str, Any]]:
    ...
    for entry in load_custody_entries(case_id=case_id):
        ...
        bucket = by_tx.setdefault(tx_id, {
            ...
            # SYBIL_CLUSTER finding - a quick "has forensic annotation" flag for the
            # browsing list, the full structured item is only in
            # custody_chain_for_transaction's detail view.
            'has_token_approval_evidence': False,
            'has_sybil_evidence': False,
        })
        bucket['access_count'] += 1
        if entry.get('token_approval_evidence'):
            bucket['has_token_approval_evidence'] = True
        if entry.get('sybil_evidence'):
            bucket['has_sybil_evidence'] = True
